...
// // ——— Get Real-Time USDC Balance (COMPREHENSIVE DEBUG) ————————————————
// async function getAvailableUSDC() {
//     try {
//         console.log('🔍 Checking wallet:', MAIN_WALLET);

//         // Method 1: Check perpetuals account
//         console.log('📊 Checking Perpetuals Account...');
//         const perpState = await sdk.info.perpetuals.getClearinghouseState(MAIN_WALLET);
//         console.log('🏦 Perpetuals State:', JSON.stringify(perpState, null, 2));

//         // Method 2: Check spot account  
//         console.log('💱 Checking Spot Account...');
//         const spotState = await sdk.info.spot.getSpotClearinghouseState(MAIN_WALLET);
//         console.log('🏪 Spot State:', JSON.stringify(spotState, null, 2));

//         // Method 3: Get all user open orders (also returns some balance info)
//         console.log('📋 Checking Open Orders...');
//         const openOrders = await sdk.info.getUserOpenOrders(MAIN_WALLET);
//         console.log('📝 Open Orders:', JSON.stringify(openOrders, null, 2));

//         // Extract balances from both accounts
...
//         // If perpetuals has money, use it
//         if (perpBalance > 0) {
//             return {
...
//         // If spot has money but perp doesn't, we need to transfer
//         if (spotUSDC > 0 && perpBalance === 0) {
//             console.log('⚠️ Funds are in SPOT account. Need to transfer to PERPETUALS for trading!');
//             console.log(`💸 Consider transferring ${spotUSDC} USDC from Spot to Perpetuals`);

//             // Return spot balance but warn user
//             return {
//                 totalUSDC: spotUSDC,
...
//         console.error('❌ No USDC found in either Perpetuals or Spot accounts!');
